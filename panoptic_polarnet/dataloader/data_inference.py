...
def process_one_frame(
    ...
    data_tuple = prepare_input_tuple(
        raw_data, grid_size, panoptic_proc, thing_list, return_ref, fixed_volume_space, max_volume_space, min_volume_space, ignore_label
    )
...
def prepare_input_tuple(
    ...
    center, center_points, offset = panoptic_proc(
        insts[mask], xyz[:np.size(labels)][mask[:, 0]], processed_inst, voxel_position[:2,:,:,0], unique_label_dict, min_bound, intervals
    )
