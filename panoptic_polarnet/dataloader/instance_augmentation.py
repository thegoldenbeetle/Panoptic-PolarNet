...
class instance_augmentation(object):
    def __init__(
        ...
        random_add=False,
        random_rotate=False,
        local_transformation=False,
    ):
        self.thing_list = thing_list
        self.instance_weight = [
            class_weight[thing_class_num - 1] for thing_class_num in thing_list
        ]
        self.instance_weight = np.asarray(self.instance_weight) / np.sum(
            self.instance_weight
        )
    ...
    def instance_aug(self, point_xyz, point_label, point_inst, point_feat=None):
        ...
        if self.random_add:
            ...
            instance_choice = np.random.choice(
                len(self.thing_list), self.add_num, replace=True, p=self.instance_weight
            )
            ...
            for n, count in zip(uni_inst, uni_inst_count):
                ...
                random_choice = np.random.choice(
                    len(self.instance_path[self.thing_list[n]]), count
                )
                # add to current scan
                for idx in random_choice:
                    points = np.fromfile(
                        self.instance_path[self.thing_list[n]][idx], dtype=np.float32
                    ).reshape((-1, 4))
                    add_xyz = points[:, :3]
                    ...
                    add_label = np.ones((points.shape[0],), dtype=np.uint8) * (
                        self.thing_list[n]
                    )
                    add_inst = np.ones((points.shape[0],), dtype=np.uint32) * (
                        add_idx << 16
                    )
                    ...
                    if point_feat is not None:
                        add_fea = points[:, 3:]
                        if len(add_fea.shape) == 1:
                            add_fea = add_fea[..., np.newaxis]
        ...
        for inst in unique_label:
            ...
            if self.random_flip:
                ...
                if flip_type == 3:
                    point_xyz[index, :2] = self.instance_flip(
                        point_xyz[index, :2],
                ...
                if random_num > 0.8 and inst & 0xFFFF > 0:
                    ...
                    for r in random_choice:
                        ...
                        if self.check_occlusion(
                            np.delete(point_xyz, index, axis=0), center_r[0]
                        ):
                            fail_flag = False
                            break
                    if not fail_flag:
