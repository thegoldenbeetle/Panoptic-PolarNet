...
class SemKITTI(data.Dataset):
    def __init__(
        self, data_path, imageset="train", return_ref=False, instance_pkl_path="data"
    ):
...
class voxel_dataset(data.Dataset):
    def __init__(
        ...
        self.panoptic_proc = PanopticLabelGenerator(
    ...
    def __getitem__(self, index):
        ...
        if self.instance_aug:
            xyz, labels, insts, feat = self.inst_aug.instance_aug(
                xyz, labels.squeeze(), insts.squeeze(), feat
            )
        ...
        grid_ind = (
            np.floor((np.clip(xyz, min_bound, max_bound) - min_bound) / intervals)
        ).astype(np.int)
        ...
        dim_array[0] = -1
        voxel_position = (np.indices(self.grid_size) + 0.5) * intervals.reshape(
            dim_array
        ...
        if inst_label.size > 1:
            ...
            # process panoptic
            processed_inst = (
                np.ones(self.grid_size[:2], dtype=np.uint8) * self.ignore_label
            )
            inst_voxel_pair = np.concatenate(
                [grid_ind[mask[:, 0], :2], inst_label[..., np.newaxis]], axis=1
            )
        ...
        center, center_points, offset = self.panoptic_proc(
            insts[mask],
            xyz[mask[:, 0]],
            processed_inst,
            voxel_position[:2, :, :, 0],
...
class spherical_dataset(data.Dataset):
    def __init__(
        ...
        self.panoptic_proc = PanopticLabelGenerator(
            self.grid_size, sigma=args["gt_generator"]["sigma"], polar=True
        )
        if self.instance_aug:
            self.inst_aug = instance_augmentation(
                self.point_cloud_dataset.instance_pkl_path + "/instance_path.pkl",
    ...
    def __getitem__(self, index):
        ...
        distance_feature[current_grid[:, 2], current_grid[:, 0], current_grid[:, 1]] = (
            1.0
        )
