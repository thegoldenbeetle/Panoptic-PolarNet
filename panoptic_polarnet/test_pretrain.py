...
def main(args):
    ...
    if args["model"]["polar"]:
        test_dataset = spherical_dataset(
            test_pt_dataset,
            args["dataset"],
            grid_size=grid_size,
            ignore_label=0,
    ...
    with torch.no_grad():
        for i_iter_val, (
            ...
            for count, i_val_grid in enumerate(val_grid):
                ...
                panoptic_labels = (
                    panoptic_labels.cpu().detach().numpy().astype(np.uint32)
                )
    ...
    for class_name, class_pq, class_sq, class_rq, class_iou in zip(
        unique_label_str, class_all_PQ[1:], class_all_SQ[1:], class_all_RQ[1:], ious[1:]
    ):
        print(
    ...
    with torch.no_grad():
        for i_iter_test, (
            ...
            test_pt_fea,
            test_index,
        ) in enumerate(test_dataset_loader):
            # predict
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p",
        "--pretrained_model",
        default="pretrained_weight/Panoptic_SemKITTI_PolarNet.pt",
    )
    parser.add_argument(
