...
from panoptic_polarnet.dataloader.dataset import (
    collate_fn_BEV,
    SemKITTI,
    SemKITTI_label_name,
    spherical_dataset,
...
def main(args):
    ...
    summary_writer = SummaryWriter(args["tensorboard_save_path"])
    ...
    if args["model"]["polar"]:
        train_dataset = spherical_dataset(
            train_pt_dataset,
            args["dataset"],
            grid_size=grid_size,
            ignore_label=0,
    ...
    while epoch < args["model"]["max_epoch"]:
        ...
        for i_iter, (
            ...
            if global_iter % check_iter == 0:
                ...
                with torch.no_grad():
                    for i_iter_val, (
                        ...
                        val_label_tensor = val_vox_label.type(torch.LongTensor).to(
                            pytorch_device
                        )
                        ...
                        for count, i_val_grid in enumerate(val_grid):
                            ...
                            panoptic_labels, center_points = get_panoptic_segmentation(
                                torch.unsqueeze(predict_labels[count], 0),
                        ...
                        del (
                            ...
                            val_grid_ten,
                            val_gt_center,
                            val_gt_center_tensor,
                            val_gt_offset,
                            val_gt_offset_tensor,
                            predict_labels,
                            center,
                            offset,
                            panoptic_labels,
                ...
                for class_name, class_pq, class_sq, class_rq, class_iou in zip(
                    ...
                    if class_name in ("person", "car", "motorcycle", "fence", "road", "parking"):
                        summary_writer.add_scalar(f"valid/{class_name}/PQ", class_pq * 100, global_step=i_iter)    
                        summary_writer.add_scalar(f"valid/{class_name}/PQ", class_pq * 100, global_step=i_iter)    
                        summary_writer.add_scalar(f"valid/{class_name}/PQ", class_pq * 100, global_step=i_iter)    
                    summary_writer.add_scalar("valid/PQ", class_PQ * 100, global_step=i_iter)
                # save model if performance is improved
                if best_val_PQ < class_PQ:
                    best_val_PQ = class_PQ
                    torch.save(my_model.state_dict(), model_save_path)
                print(
                    "Current val PQ is %.3f while the best val PQ is %.3f"
                    % (class_PQ * 100, best_val_PQ * 100)
                )
                print("Current val miou is %.3f" % (miou * 100))
                summary_writer.add_scalar("valid/PQ", class_PQ * 100, global_step=i_iter)
                summary_writer.add_scalar("valid/miou", miou * 100, global_step=i_iter)
                ...
                if start_training:
                    ...
                    summary_writer.add_scalar("loss/loss", loss, global_step=i_iter)
                    summary_writer.add_scalar("loss/semantic_loss", loss, global_step=i_iter)
                    summary_writer.add_scalar("loss/heatmap_loss", loss, global_step=i_iter)
                    summary_writer.add_scalar("loss/offset_loss", loss, global_step=i_iter)
                    summary_writer.add_scalar("epoch", epoch, global_step=i_iter)
            ...
            try:
                ...
                train_grid_ten = [
                    torch.from_numpy(i[:, :2]).to(pytorch_device) for i in train_grid
                ]
                train_label_tensor = train_label_tensor.type(torch.LongTensor).to(
                    pytorch_device
                )
                ...
                if (
                    args["model"]["enable_SAP"]
                    and epoch >= args["model"]["SAP"]["start_epoch"]
                ):
                ...
                if (
                    ...
                    for i, fea in enumerate(train_pt_fea_ten):
                        ...
                        top_k_grad, _ = torch.topk(
                            fea_grad,
                            int(args["model"]["SAP"]["rate"] * fea_grad.shape[0]),
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--tensorboard_save_path", default="./tensorboard_logs")
    parser.add_argument(
