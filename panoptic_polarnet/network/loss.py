...
class panoptic_loss(torch.nn.Module):
    def __init__(
        ...
        self.lost_dict = {"semantic_loss": [], "heatmap_loss": [], "offset_loss": []}

    def reset_loss_dict(self):
        self.lost_dict = {"semantic_loss": [], "heatmap_loss": [], "offset_loss": []}

    def forward(
        self, prediction, center, offset, gt_label, gt_center, gt_offset, save_loss=True
    ):
        # semantic loss
        loss = lovasz_softmax(
            torch.nn.functional.softmax(prediction), gt_label, ignore=255
        ) + self.CE_loss(prediction, gt_label)
        if save_loss:
