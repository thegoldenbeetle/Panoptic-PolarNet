...
class ptBEVnet(nn.Module):
    ...
    def forward(self, pt_fea, xy_ind, voxel_fea=None, pp_model=None, bev_model=None):
        if pp_model is None:
            pp_model = self.PPmodel
        if bev_model is None:
            bev_model = self.BEV_model
        ...
        if self.pt_selection == "random":
            grp_ind = grp_range_torch(unq_cnt, cur_dev)[
            ...
            remain_ind = np.zeros((pt_num,), dtype=bool)
        ...
        if self.pt_model == "pointnet":
            processed_cat_pt_fea = pp_model(cat_pt_fea)


        if self.pt_pooling == "max":
            pooled_data = torch_scatter.scatter_max(
                processed_cat_pt_fea, unq_inv, dim=0,
            )[0]
        else:
            raise NotImplementedError
        ...
        sem_prediction, center, offset = bev_model(out_data)
...
def nb_greedy_FPS(xyz, K):
    ...
    pairwise_distance = (
        sum_vec + np.transpose(sum_vec) - 2 * np.dot(xyz, np.transpose(xyz))
    )
