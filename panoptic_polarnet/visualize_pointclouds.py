    ...
    if len(point_clouds) == 1:
        ...
        while True:
            ...
            if current_time - last_update_time > update_interval:
                ...
                if color_option == 'r':
                    colors = [[0, 0, reflectance] for reflectance in point_clouds[frame_index][:, 3]]
                else:
                    colors = get_colors(labels[frame_index], color_option, inst_color_lut, sem_color_lut)
                point_cloud.colors = o3d.utility.Vector3dVector(colors)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--color', choices=['r', 's', 'i'], default="r")   # reflectance semantic instance
    ...
    if args.path:
        ...
        pointclouds = [np.fromfile(x, dtype=np.float32).reshape((-1, 4)) for x in points_files]
